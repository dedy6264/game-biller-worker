...
var respPay = map[string]mainResponse{
	...
	// FAILED
	"13":  {"CUSTOMER NUMBER BLOCKED", "", CodeFailed},
	"14":  {"INCORRECT DESTINATION NUMBER", "", CodeFailed},
	"16":  {"NUMBER NOT MATCH WITH OPERATOR", "", CodeFailed},
	"18":  {"NUMBER NOT AVAILABLE", "", CodeFailed},
	"19":  {"NUMBER IS ALREADY IN USE", "", CodeFailed},
	"20":  {"CODE NOT FOUND", "", CodeInvalidProduct},
	"21":  {"NUMBER EXPIRED", "", CodeFailed},
	"121": {"MONTHLY TOP UP LIMIT EXCEEDED", "", CodeMaxTrx},
	"131": {"TOP UP REGION BLOCKED FOR PLAYER", "Your current destination number top up request is blocked in that region. Please try again with a different destination number.", CodeFailed},
	"132": {"PRODUCT CODE NOT ELIGIBLE DUE TO SUBSCRIBER LOCATION", "", CodeInvalidProduct},
	"202": {"MAXIMUM 1 NUMBER 1 TIME IN 1 DAY", "", CodeMaxTrx},
	"203": {"NUMBER IS TOO LONG", "", CodeFailed},
	"206": {"THIS DESTINATION NUMBER HAS BEEN BLOCKED", "", CodeFailed},
	"207": {"MAXIMUM 1 NUMBER WITH ANY CODE 1 TIME IN 1 DAY", "", CodeMaxTrx},

	"17":  {"INSUFFICIENT DEPOSIT", "", CodeFailed},
	"301": {"EMAIL SEND LIMIT REACHED", "", CodeFailed},
	"07":  {"FAILED", "Your current transaction has failed. Please try again.", CodeFailed},
	"12":  {"BALANCE MAXIMUM LIMIT EXCEEDED", "", CodeFailed},
	"92":  {"GENERAL ERROR", "Upstream provider responded with an unmapped error structure or critical raw payload exception.", CodeFailed},
	"93":  {"INVALID AMOUNT", "The destination identity string failed regex or basic character formatting check.", CodeFailed},
	"100": {"INVALID SIGNATURE", "Invalid client key, invalid signature hash, or IP address not registered in the whitelist.", CodeFailed},
	"101": {"INVALID COMMAND", "The API endpoint requested does not support the HTTP verb or request method applied.", CodeFailed},
	"102": {"INVALID IP ADDRESS", "Invalid client key, invalid signature hash, or IP address not registered in the whitelist.", CodeFailed},
	"106": {"PRODUCT IS TEMPORARILY OUT OF SERVICE", "The upstream vendor gateway for this specific product is currently undergoing maintenance.", CodeFailed},
	"107": {"XML / JSON FORMAT ERROR", "The destination identity string failed regex or basic character formatting check.", CodeFailed},
	"117": {"PAGE NOT FOUND", "The API endpoint requested does not support the HTTP verb or request method applied.", CodeFailed},
	"204": {"WRONG AUTHENTICATION", "Invalid client key, invalid signature hash, or IP address not registered in the whitelist.", CodeFailed},
	"205": {"WRONG COMMAND", "The API endpoint requested does not support the HTTP verb or request method applied.", CodeFailed},
	"141": {"INVALID USER ID / ZONE ID / SERVER ID / ROLENAME", "External game validator service rejected the provided Player ID, Zone ID, or Server ID.", CodeInvalidCustID},
	"142": {"INVALID USER ID", "External game validator service rejected the provided Player ID, Zone ID, or Server ID.", CodeInvalidCustID},
	"143": {"INQUIRY NOT NEEDED", "Payment request rejected. The referenced transaction is not in INQUIRY_SUCCESS status and cannot be processed.", CodeFailed},
}
